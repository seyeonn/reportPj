...
@Service
public class UploadedFileService { // 업로드된 파일의 내용을 uploadedFile 테이블의 data 필드에 저장한다.

    @Autowired 
    UploadedFileMapper uploadedFileMapper;

    // 파일 목록 조회
    public List<UploadedFile> findAllInPosting(int id) {
        return uploadedFileMapper.findAllInPosting(id); // uploadedFile 테이블의 모든 레코드를 조회
    }

    // 파일 저장
    public void save(MultipartFile multipartFile,int id) throws IOException {
        // 업로드된 파일의 이름을 구한다.
